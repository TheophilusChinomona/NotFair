...
import { GOOGLE_ADS_TOOLS } from "@/server/mcp-google-ads/tools";
...
/**
 * MCP server for Google Ads API tools (GAQL search, campaign pause/enable).
 *
 * Auth: Bearer token verified against the shared MCP server secret
 * (same as orchestration/browser/GSC). The Google Ads API itself is
 * authenticated via ADC (Application Default Credentials) and the
